...
drawbar(Monitor *m)
{
	...
	if (m == selmon || 1) { /* status is only drawn selected monitor */
...
bstack(Monitor *m) {
	...
	for (i = 0, mx = m->gappx, tx = m->wx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
...
bstackhoriz(Monitor *m) {
	...
	for (i = 0, mx = m->gappx, tx = m->wx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, tx + m->gappx, ty + m->gappx, m->ww - (2 * c->bw) - 2*m->gappx, th - (2 * c->bw) - 2*m->gappx, 0);
			if (th != m->wh)
				ty += HEIGHT(c) + m->gappx;
		}
	}
...
gaplessgrid(Monitor *m) {
	...
	for(i = 0, c = nexttiled(m->clients); c; i++, c = nexttiled(c->next)) {
		...
		cx = m->wx + cn*cw;
		cy = m->wy + rn*ch;
		resize(c, cx + m->gappx, cy + m->gappx, cw - 2 * c->bw - 2*m->gappx, ch - 2 * c->bw - 2*m->gappx, False);
		rn++;
		if(rn >= rows) {
			rn = 0;
			cn++;
